...
import { Award, Search, Calendar, FileText, CheckCircle2, Download, Printer, Filter, ShieldCheck, Sparkles, Loader2, ArrowRight } from 'lucide-react';
...
export default function PengumumanPage() {
  ...
  const isUserAccepted = myApp?.status === 'DITERIMA' || myApp?.status === 'PENCAIRAN_TERMIN_1' || myApp?.status === 'PENCAIRAN_TERMIN_2';

  return (
    <div className="w-full space-y-8 max-w-6xl mx-auto pb-16">
      {/* Single Dynamic Header Banner */}
            ...
            {isUserAccepted ? (
              ...
              <>
                <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-white/10 text-blue-200 text-xs font-semibold">
                  <span>Pengumuman Resmi • TA 2026</span>
                </div>
                <h1 className="text-2xl sm:text-4xl font-black tracking-tight">
                  Hasil Seleksi &amp; Penetapan Penerima Beasiswa
                </h1>
                <p className="text-blue-100 text-xs sm:text-sm leading-relaxed font-light">
                  Pengumuman resmi Keputusan Kepala Dinas Pendidikan dan Kebudayaan Provinsi Sulawesi Tenggara tentang Penetapan Mahasiswa Penerima Beasiswa Stimulan Sultra Cerdas Tahun 2026.
                </p>
              </>
            )}
          </div>
      ...
      {/* Sanggahan Alert Banner (jika DITOLAK) */}
      {myApp?.status === 'DITOLAK' && (
